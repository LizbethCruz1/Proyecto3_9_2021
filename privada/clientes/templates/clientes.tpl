			...
			<form name="formModif{$r.id_cliente}" method="post" action="cliente_modificar.php">
				...
				<td align="center">
					<form name="formElimi{$r.id_cliente}" method="post" action="clientes_eliminar.php">
						<input type="hidden" name="id_cliente" value="{$r.id_cliente}">
						<a href="javascript:document.formElimi{$r.id_cliente}.submit();" title="Eliminar Cliente Sistema" onclick="javascript:return(confirm(' Desea realmente Eliminar al Cliente: {$r.nombre} {$r.ap} {$r.am} ?')); location.href='clientes_eliminar.php'">
							Eliminar>>
						</a>
					</form>
				</td>
